...
def transform():
    try:
        ...
        magic_image_path = model.settings.join_path(model.settings.MEDIA_DIR, filename)
...
def prediction():
    # load the wrap image
    wrap_image_filepath = model.settings.join_path(model.settings.MEDIA_DIR, 'magic_color.jpg') 
    image = cv2.imread(wrap_image_filepath)
    image_bb, results = pred.getPredictions(image)
    
    bb_filename = model.settings.join_path(model.settings.MEDIA_DIR, 'bounding_box.jpg') 
